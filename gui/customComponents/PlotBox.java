...
import fr.inria.contraintes.biocham.customComponents.Plot.Format;
...
/**
 * Class that creates a custom JPanel component that can hold a Plot image.
 *  
 */ 
public class PlotBox extends JPanel {
    ///////////////////////////////////////////////////////////////////
    ////                         constructor                       ////

	Vector<Format> _formats;
     ...
     */
    public synchronized BufferedImage exportImage(Vector<Format> formats) {
    	boolean b1=SwingUtilities.isEventDispatchThread();
    	_formats=formats;
    ...
    /**
     * My method, combination of two: formats+size as an input!
     * @author Dragana Jovanovska
     */
    public synchronized BufferedImage exportImage(Vector<Format> formats,Rectangle rectangle) {
    	_formats=formats;
    	return exportImage(new BufferedImage(rectangle.width, rectangle.height,
                BufferedImage.TYPE_INT_ARGB), rectangle,
                _defaultImageRenderingHints(), false);
    ...
    public synchronized BufferedImage exportImage(BufferedImage bufferedImage,
        ...
        _drawPlot(graphics, false, rectangle,_formats);
...
*/
     ...
     *  @see #getColors()
     */
    public synchronized void setColors(ArrayList<Color> colors) {
        // Changing legend means we need to repaint the offscreen buffer.
        _plotImage = null;
        _colors = colors;
    }
    ...
    protected synchronized void _drawPlot(Graphics graphics, boolean clearfirst) {
        Rectangle bounds = getBounds();
        _drawPlot(graphics, clearfirst, bounds,_formats);
    }
    ...
    protected synchronized void _drawPlot(Graphics graphics,
            boolean clearfirst, Rectangle drawRect,Vector<Format> fmts) {
    		_formats=fmts;
    ...
    protected void _help() {
        String message = "Ptolemy plot package\n"
                ...
                + "Version "
                + PTPLOT_RELEASE
                + ", Build: $Id: PlotBox.java,v 1.273 2006/03/29 00:01:46 cxh Exp $\n\n"
                + "Key bindings:\n"
    ...
    // distinct data sets.
    static protected ArrayList<Color> _colors = new ArrayList<Color>(Arrays.asList(
        ...
        while (v.hasMoreElements()) {
            ...
            if (dataset >= 0) {
                if (_usecolor) {
                	...
                	int siz=_formats.size();
                	if(_formats.size()>0){
                		if(_formats.get(dataset)!=null){
                			if(_formats.get(dataset).color!=null){
                				graphics.setColor(_formats.get(dataset).color);
                				t=true;
                			}
                		}
                	}
                	if(!t){
                		int color = dataset % _colors.size();
                        graphics.setColor(_colors.get(color));
                	}
