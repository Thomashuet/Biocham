...
import fr.inria.contraintes.biocham.plotting.PlotPoint;
import fr.inria.contraintes.biocham.utils.EPSGraphics;
...
/**
MODIFIED!!! A flexible signal plotter.  The plot can be configured and data can
...
@version $Id: Plot.java,v 1.239 2006/03/29 00:01:47 cxh Exp $
...
/**
 * Class that draws the datasets on a plotbox(custom JPanel component) with X and Y axis.
 * 
 */ 
...
/** Add a legend (displayed at the upper right) for the specified
 ...
 public synchronized void addLegend(int dataset, String legend) {
	 boolean b1=SwingUtilities.isEventDispatchThread();
  ...
  */
 public synchronized void clear(final int dataset,final boolean forAlways) {
     Runnable doClear = new Runnable() {
         public void run() {
             _clear(dataset,forAlways);
         }
     };
 ...
 public synchronized void setMarksStyle(String style) {
     ...
     if (style.equalsIgnoreCase("none")) {
         ...
         _marks = 4;
     } else if (style.equalsIgnoreCase("crosses")) {
         _marks = 15;
     } else if(style.equalsIgnoreCase("filled_circle")){
    	 _marks = 30;
     }else if(style.equalsIgnoreCase("square")){
    	 _marks = 32;
     }else if(style.equalsIgnoreCase("filled_triangle")){
    	 _marks = 33;
     }else if(style.equalsIgnoreCase("diamond")){
    	 _marks = 34;
     }else if(style.equalsIgnoreCase("circle")){
    	 _marks = 35;
     }else if(style.equalsIgnoreCase("plus_sign")){
    	 _marks = 36;
     }else if(style.equalsIgnoreCase("filled_square")){
    	 _marks = 37;
     }else if(style.equalsIgnoreCase("triangle")){
    	 _marks = 38;
     }else if(style.equalsIgnoreCase("filled_diamond")){
    	 _marks = 39;
     }     
 ...
 protected synchronized void _drawPlot(Graphics graphics,
         boolean clearfirst, Rectangle drawRectangle,Vector<Format> fmts) {
     // We must call PlotBox._drawPlot() before calling _drawPlotPoint
     // so that _xscale and _yscale are set.
     super._drawPlot(graphics, clearfirst, drawRectangle,fmts);
  ...
  * be called via deferIfNecessary().
  */
 private void _clear(int dataset,boolean forAlways) {
     // Ensure replot of offscreen buffer.
     _plotImage = null;
     _checkDatasetIndex(dataset);
     _xyInvalid = true;
 ...
 private void _drawPlotPoint(Graphics graphics, int dataset, int index) {
     ...
     //fmt.color.connected = on;
     //fmt.connectedUseDefault = false;
     // Set the color
     if (_usecolor) {
         //int color = fmt.color;//dataset % _colors.length;
    	 counter++;
    	 if(fmt.color==null){
    		 fmt.color=_colors.get(dataset%_colors.size());
    	 }   
         graphics.setColor(fmt.color);//_colors[color]);
     } else {
